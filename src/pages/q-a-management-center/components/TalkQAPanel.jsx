...
/**
 * Talk-based Q&A Panel Component
 * Displays questions and answers using Nextcloud Talk API
 */
      ...
      <div className="space-y-3">
        ...
        {message.answers?.length > 0 && (
          <div className="ml-4 border-l-2 border-border pl-4 space-y-2">
            {message.answers.map((reply) => (
              <div key={reply.id} className="bg-muted/30 rounded-lg p-3">
                <div className="flex items-center space-x-2 mb-1">
                  <span className="text-sm font-medium">{reply.actorDisplayName}</span>
            ...
            !showReplyForm ? (
              ...
              <form onSubmit={handleSubmitReply} className="space-y-2">
                ...
                <div className="flex justify-end space-x-2 rtl:space-x-reverse">
                  ...
                  >
                    <Icon name="Send" size={16} className="mr-1 rtl:ml-1 rtl:mr-0" />
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {rooms.map((room) => {
          ...
          return (
            <Card key={room.token} className="p-4 hover:shadow-lg transition-all duration-200 cursor-pointer hover:border-primary/30 hover:bg-muted/20" 
                ...
                <div className="flex items-center space-x-2 rtl:space-x-reverse">
                  ...
                  <div>
                    <div className="flex items-center space-x-1 mb-1">
                      <h4 className="font-medium truncate max-w-28">{room.displayName}</h4>
                      {roomPermissions?.isReadOnly && (
                        <Icon name="Lock" size={12} className="text-orange-500" />
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground">
                      {room.type === 2 ? t('room_selection.room_types.group_chat', { defaultValue: 'Group Chat' }) : room.type === 6 ? t('room_selection.room_types.personal', { defaultValue: 'Personal' }) : room.type === 4 ? t('room_selection.room_types.changelog', { defaultValue: 'Changelog' }) : t('room_selection.room_types.public', { defaultValue: 'Public' })}
